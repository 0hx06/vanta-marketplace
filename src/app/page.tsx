  ...
  return (
    <SiteShell>
              ...
              <div className="rounded-[21px] border border-white/10 bg-[#17191f] p-5 text-white">
                ...
                <div className="mt-5 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="mt-2 text-2xl font-bold">745 MAD</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {productCatalog.slice(0, 4).map((product) => (
            <Link key={product.id} href={`/product/${product.id}`} className="group rounded-[24px] border border-white/10 bg-[#121419] p-4 transition hover:-translate-y-1 hover:border-[#ff4655]/60">
              ...
              <div className="mt-4 flex items-end justify-between gap-3">
                ...
                <span className="text-lg font-bold text-white">{Math.round(product.price * 10.8)} MAD</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#ff6975]">Comptes</p>
            <h2 className="mt-2 text-3xl font-bold text-white">Meilleures ventes</h2>
          </div>
        </div>
        <div className="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {productCatalog.slice(0, 4).map((product) => (
            <Link key={product.id} href={`/product/${product.id}`} className="vanta-border rounded-[28px] bg-[#111318] p-4 transition hover:-translate-y-1 hover:border-[#ff4655]/45 hover:shadow-[0_20px_60px_rgba(255,70,85,0.12)]">
              ...
              <div className="mt-5 flex items-center justify-between">
                <span className="text-2xl font-bold text-white">{Math.round(product.price * 10.8)} MAD</span>
                <span className="rounded-full bg-[#ff4655] px-3 py-1.5 text-xs font-medium text-white">Voir l'offre</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold text-white">Des vendeurs de confiance</h2>
          </div>
        </div>
        <div className="grid gap-6 md:grid-cols-3">
          {sellers.map((seller) => (
            <Link key={seller.id} href={`/seller/${seller.id}`} className="vanta-border rounded-[28px] bg-[#111318] p-5 transition hover:border-[#ff4655]/40">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {seller.verified && <span className="rounded-full bg-emerald-500/20 px-2 py-0.5 text-[10px] text-emerald-300">Vérifié</span>}
                  </div>
